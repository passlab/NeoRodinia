/*
 * Level 3: Advanced SIMD with mannualy specify the simd length
 * The most advanced SIMD version focuses on manual optimizations.
 *
 */
#include "hotspot.h"
    ...
    for ( chunk = 0; chunk < num_chunk; ++chunk ) {
        ...
        if ( r_start == 0 || c_start == 0 || r_end == grid_rows || c_end == grid_cols ) {
            for ( r = r_start; r < r_start + BLOCK_SIZE_R; ++r ) {
                #pragma omp simd aligned(temp, power, result : 32) simdlen(8)
