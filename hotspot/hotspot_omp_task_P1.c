/*
 * Level 1: Basic Task Parallelism
 * This version introduces task parallelism using #pragma omp taskloop directives to split computations into tasks. Each grid block is handled as a separate task.
 *
 */
#include "hotspot.h"
...
#pragma omp single
        {
            for ( chunk = 0; chunk < num_chunk; ++chunk ) {
                ...
                if ( r_start == 0 || c_start == 0 || r_end == grid_rows || c_end == grid_cols ) {
                    #pragma omp taskloop shared(result, temp, power)
