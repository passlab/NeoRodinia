...
void single_iteration(FLOAT *result, FLOAT *temp, FLOAT *power, int grid_rows, int grid_cols, FLOAT Cap_1, FLOAT Rx_1, FLOAT Ry_1, FLOAT Rz_1, FLOAT step) {
    ...
    #pragma omp parallel for shared(power, temp, result) private(chunk, r, c, delta) firstprivate(grid_rows, grid_cols, num_chunk, chunks_in_grid_rows) schedule(dynamic, 128)
