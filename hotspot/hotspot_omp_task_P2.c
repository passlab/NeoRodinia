/*
 * Level 2: Dynamic Task Assignment and Priorities
 * This version assigns tasks dynamically to balance the workload across threads. It uses OpenMP’s task scheduling capabilities with priority to prioritize critical tasks or those likely to be completed faster, ensuring efficient load balancing and reduced idle time.
 *
 */
#include "hotspot.h"
    ...
    #pragma omp single
    {
        for (chunk = 0; chunk < num_chunk; ++chunk) {
            ...
            if (r_start == 0 || c_start == 0 || r_end == grid_rows || c_end == grid_cols) {
                #pragma omp task shared(result, temp, power) priority(task_priority)
                {
                    for (r = r_start; r < r_end; ++r) {
                        for (c = c_start; c < c_end; ++c) {
                            ...
                            // Add similar logic for other edges and corners here...
