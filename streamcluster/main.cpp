...
float pFL(Points *points, int *feasible, int numfeasible, float z, long *k,
    ...
    while (change / cost > 1.0 * e) {
        ...
        for (i = 0; i < iter; i++) {
            ...
            change += pgain_kernel(feasible[x], points, z, k, pid);
            c++;
...
int main(int argc, char **argv) {
    ...
    if (argc < 10) {
        ...
        fprintf(stderr, "\n");
        fprintf(stderr,
                "if n > 0, points will be randomly generated instead of "
                "reading from infile.\n");
        exit(1);
    }
    kmin = atoi(argv[1]);
    ...
    double kernel_time = streamCluster_wrapper(
        stream, kmin, kmax, dim, chunksize, clustersize, outfilename);
