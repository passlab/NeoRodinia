/*
 * Level 3: Advanced SIMD with Explicit Vector Length
 * At this level, explicit vector length (simdlen) is defined to maximize SIMD utilization. This ensures that a specific number of elements are processed in each SIMD instruction.
 *
 */
#include "sc.h"
// #include "sc_kernel.h"
...
#pragma omp simd simdlen(16) aligned(coord_d:64) aligned(d_points:64) reduction(+:*cost_of_opening_x)
    for (i = k1; i < k2; i++) {
        float x_cost =
...
#pragma omp simd simdlen(16) aligned(coord_d:64) aligned(d_points:64) 
        for (int i = k1; i < k2; i++) {
