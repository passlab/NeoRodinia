/*
 * Level 2: Aligned SIMD Parallelism
 * Memory alignment and compiler optimizations are enforced for better performance. This ensures that memory accesses are aligned to the cache line, reducing overhead.
 *
 */
#include "sc.h"
// #include "sc_kernel.h"
...
#pragma omp simd aligned(coord_d:64) aligned(d_points:64) reduction(+:*cost_of_opening_x)
    for (i = k1; i < k2; i++) {
        float x_cost =
...
#pragma omp simd aligned(coord_d:64) aligned(d_points:64) 
        for (int i = k1; i < k2; i++) {
