/*
 * Level 2: Taskloop with Grain Size and Dependencies
 * This level adds granularity control via grainsize to balance workloads better and ensures correctness using depend clauses to manage dependencies between tasks.
 *
 */
#include "sc.h"
// #include "sc_kernel.h"
...
double pgain_kernel(long x, Points *points, double z, long int *numcenters,
    ...
    */
    ...
    #pragma omp parallel
    {
        ...
        for (i = k1; i < k2; i++) {
            #pragma omp task depend(in: points->p[i]) depend(out: switch_membership[i])
            {
                float x_cost =
