/*
 * The kernel divides the matrix into blocks of size `BS x BS` and performs LU factorization on each block. This approach can improve data locality and reduce the impact of memory access latency, especially for large matrices.
 *
 */
#include <stdio.h>
#include "lud.h"
...
void lud_kernel(float* a, int size) {
    ...
    for (offset = 0; offset < size - BS; offset += BS) {
        ...
        #pragma omp parallel for schedule(auto) private(chunk_idx) shared(size, chunks_per_inter, chunks_in_inter_row, offset, a)
