/*
 * The kernel divides the matrix into blocks of size `BS x BS` and performs LU factorization on each block.
 * This approach can improve data locality and reduce the impact of memory access latency, especially for large matrices.
 * The inner loop is parallelized using OpenMP's target parallel for directive, distributing the workload across available threads.
 */
#include <stdio.h>
#include <omp.h>
...
void lud_kernel(float* a, int size) {
    ...
    for (offset = 0; offset < size - BS; offset += BS) {
        ...
        #pragma omp target parallel for map(to:size) map(tofrom:a[0:size*size]) private(chunk_idx) shared(size, chunks_in_inter_row, offset, a)
