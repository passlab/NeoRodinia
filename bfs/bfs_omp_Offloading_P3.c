...
#pragma omp target data map(to : no_of_nodes, h_graph_mask [0:no_of_nodes],    \
    ...
    do {
        ...
        stop = false;
        #pragma omp target teams distribute parallel for num_teams(NUM_TEAMS) num_threads(TEAM_SIZE)
        ...
        #pragma omp target teams distribute parallel for map(tofrom : stop) num_teams(NUM_TEAMS) num_threads(TEAM_SIZE)
        for (int tid = 0; tid < no_of_nodes; tid++) {
